...
//
$mail_sys = "zkwkttns@gmail.com";	// 管理者のメールアドレス
$from_name = "OiC IKAYAKI CS部";	// メール送信者の表示
$from_mail = "OiC_IKAYAKI@contuct-us.ac.jp";	// メール送信者のメールアドレス（返信先）
$user_mail = "item3";	// 利用者にメールを送る場合のメールアドレス項目
//---
$title = "お問い合わせ";
$subject = "お問い合わせ確認\n";
$body = "次の内容でお問い合わせを受け付けました。
担当者からの返信があるまでしばらくお待ちください。
...
//
$form_input = array(
	"item1" => array("title" => "●氏名", "name" => "item1", "func" => "2", "require" => "1", "check" => "1",),
	"item2" => array("title" => "●フリガナ", "name" => "item2", "func" => "2", "require" => "1", "check" => "1",),
	"item3" => array("title" => "●メールアドレス", "name" => "item3", "func" => "2", "require" => "1", "check" => "3",),
	"item4" => array("title" => "●メールアドレス（確認）", "name" => "item4", "func" => "2", "require" => "1", "check" => "4",),
	"item5" => array("title" => "●お問い合わせ項目", "name" => "item5", "func" => "3", "require" => "1", "check" => "1", "list"=>array("1" => "本校学祭について", "2" => "OCスタッフについて", "3" => "本店舗について", "4" => "このサイトについて", "5" => "その他のお問い合わせ", )),
	"item6" => array("title" => "●お問い合わせ内容", "name" => "item6", "func" => "7", "require" => "1", "check" => "1",),
	"item7" => array("title" => "●マクロ等を利用したキャプチャチェック", "name" => "item7", "func" => "9", "require" => "1", "check" => "6",),
...
if (!$_REQUEST["mode"]) {
	...
	foreach ($form_input as $key => $val) {
		if ($val["func"] == 13) {	// File
			$mail_body .= "●お名前" . $val["title"] . "：" . $form[$val["name"] . "_file"] . "\n";
		} else if ($val["func"] == 10) {
			$mail_body .= "●お名前" . $val["title"] . "：" . $pref_list[$form[$val["name"] . "_pref"]] . $form[$val["name"] . "_address"] . "\n";
		} else if ($val["func"] == 11) {
			$mail_body .= "●お名前" . $val["title"] . "：" . $form[$val["name"] . "_year"] . "年" . 
				$form[$val["name"] . "_month"] . "月" . $form[$val["name"] . "_day"] . "日\n";
		} else if ($val["func"] == 12) {
			$mail_body .= "●お名前" . $val["title"] . "：" . $form[$val["name"] . "_month"] . "月" . $form[$val["name"] . "_day"] . "日\n";
		} else if ($val["func"] == 3) {	// 単一選択（ラジオボタン）
			$mail_body .= "●お名前" . $val["title"] . "：" . $form_input[$val["name"]]["list"][$form[$val["name"]]] . "\n";
			...
			if ($form[$val["name"]]) {
				...
				$mail_body .= "●お名前" . $val["title"] . "：" . implode("、", $ary) . "\n";
			}
		} else if ($val["func"] == 5) {	// 選択（プルダウン）
			$mail_body .= "●お名前" . $val["title"] . "：" . $form_input[$val["name"]]["list"][$form[$val["name"]]] . "\n";
		} else if ($val["func"] == 6) {	// 複数個1行テキスト入力
			$mail_body .= "●お名前" . $val["title"] . "：\n";
			...
			$mail_body .= "●お名前" . $val["title"] . "：" . $form[$val["name"]] . "\n";
		}
	}
